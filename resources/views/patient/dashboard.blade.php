<x-layouts.dashboard>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('patient.symptoms.index') }}" class="btn btn-primary shadow-sm"><i class="bi bi-robot"></i>
            Symptom Analyzer</a>
    </div>
    ...
    <div class="row g-4 mb-4">
        <div class="col-md-3">
            <div class="card">
                ...
                <p class="fs-2 fw-bold">{{ $stats['total'] }}</p>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card">
                <h5>Upcoming</h5>
                <p class="fs-2 fw-bold text-warning">{{ $stats['upcoming'] }}</p>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card">
                <h5>Completed</h5>
                <p class="fs-2 fw-bold text-success">{{ $stats['completed'] }}</p>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card">
                <h5>Cancelled</h5>
                <p class="fs-2 fw-bold text-danger">{{ $stats['cancelled'] }}</p>
            </div>
        </div>
    </div>
            ...
            @forelse($upcomingAppointments as $appointment)
                <li class="list-group-item d-flex justify-content-between align-items-center p-3">
                    <div>
                        ...
                        <span
                            class="badge bg-warning-subtle text-warning-emphasis rounded-pill text-capitalize">{{ $appointment->status }}</span>
                        @if($appointment->token_number)
                            <span class="badge bg-success-subtle text-success-emphasis rounded-pill">Token:
                                {{ $appointment->token_number }}</span>
                        @endif
                        <span
                            class="ms-2 text-muted">{{ \Carbon\Carbon::parse($appointment->appointment_date)->format('Y-m-d \a\t h:i A') }}</span>
                    </div>
                    ...
                    <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal"
                        ...
                        data-doctor="{{ $appointment->doctor_name }}" data-specialty="{{ $appointment->doctor_specialty }}"
                    ...
                    modalBody.innerHTML = `
                                    <div class="p-2">
                                        <div class="row text-center mb-3">
                                            ...
                                            ${token != 'N/A' && token != null && token != '' ? `<div class="col"><span class="badge fs-6 bg-success text-white">Token: ${token}</span></div>` : ''}
                                        </div>
